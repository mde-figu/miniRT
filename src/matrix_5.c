...
/*   Updated: 2021/05/27 13:53:31 by mde-figu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../include/components.h"

static void init(double *v, double *v1, double *v2, t_tuple double)
{
	double	angle;
...
t_matrix	rotate(t_tuple normal)
{
	...
	write_matrix(&a, 0, 0, vals[0] + (ft_power_of(axis.x, 2) * vals[2]));
	...
	write_matrix(&a, 1, 1, vals[0] + (ft_power_of(axis.y, 2) * vals[2]));
	...
	write_matrix(&a, 2, 2, vals[0] + (ft_power_of(axis.z, 2) * vals[2]));
